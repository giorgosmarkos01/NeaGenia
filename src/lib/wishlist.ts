...
export async function getWishlistProducts(userId: string): Promise<Product[]> {
  const [rows]: any[] = await db.query(
    `
    SELECT 
      i.id, i.name, i.slug, i.product_code, i.hs_code,
      i.category_id, i.group_id, i.stock, i.stock_status,
      i.collaborator_id, i.description_short, i.description_full,
      i.specifications, i.price, i.weight, i.highlight,
